defmodule DragnCardsGame.GameUiTest do
  ...
  test "Undo", %{gameui: gameui, user: user, plugin: plugin} do
    ...
    assert gameui["relayToLog"]["messages"] == ["Action 1"]
    ...
    assert gameui["replayLength"] == 2
    assert gameui["relayToLog"]["messages"] == ["--- Undo ---", "Action 1", "--- End of undo ---"]
    ...
    assert gameui["game"]["roundNumber"] == 1
    assert gameui["replayLength"] == 2
    assert gameui["relayToLog"]["messages"] == ["--- Redo ---", "Action 1", "--- End of redo ---"]
    ...
    assert gameui["replayLength"] == 2
    assert gameui["relayToLog"]["messages"] == ["Action 2"]
